...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Welcome {user.first_name}! Your account has been "
...
def profile_view(request):
    ...
    upcoming_bookings = (
        Booking.objects.filter(user=request.user, booking_date__gte=today)
        .exclude(status="Cancelled")
        .count()
    )

    completed_count = Booking.objects.filter(
        user=request.user, booking_date__lt=today, status="Confirmed"
    ).count()
...
def check_email(request):
    ...
    if not email:
        return JsonResponse(
            {"available": False, "message": "Please enter an email address."}
        )
