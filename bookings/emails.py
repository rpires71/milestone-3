...
def send_booking_update_email(booking, change_type="updated"):
    ...
    try:
        ...
        if change_type == "cancelled":
            subject = (
                f"Booking Cancelled - {booking.reference_number} - Portuguese Kitchen"
            )
            template_html = "emails/booking_cancelled.html"
            template_txt = "emails/booking_cancelled.txt"
        else:
            subject = (
                f"Booking Updated - {booking.reference_number} - Portuguese Kitchen"
            )
        ...
        logger.info(
            f"Update email sent successfully for booking {booking.reference_number}"
        )
        return True
